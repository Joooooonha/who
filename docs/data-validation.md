...
3. 후보자별 공약 조회
   ...
   - `prmsRealmNameN`, `prmsTitleN`, `prmmContN` 확인
...
후보자 정보 API 단계가 계속 비어 있으면 문서 예시 `cnddtId`로 공약 API만 직접 확인한다.

```sh
java tools/nec-probe/NecDataProbe.java --target docs-sample --candidate-id 1000000000
```

모든 선거코드 조합의 공약 데이터 존재 여부를 샘플링하려면 다음을 실행한다.

```sh
java tools/nec-probe/NecDataProbe.java --target pledge-availability
```

결과는 `data/probes/pledge-availability/pledge-availability.md`에 저장된다. 이 리포트는 각 선거코드 조합에서 후보자 최대 3명을 샘플링하므로, "공약 데이터가 붙는 선거종류 후보군"을 찾기 위한 1차 진단용이다.
